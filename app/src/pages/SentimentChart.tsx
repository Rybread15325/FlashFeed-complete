...
export function SentimentChart({ data }: Props) {
  ...
  useEffect(() => {
    ...
    let disposed = false
    import('lightweight-charts').then(({ createChart, ColorType, HistogramSeries, LineSeries }) => {
      ...
      const series = chart.addSeries(HistogramSeries, { color: '#334155' })
      ...
      // Zero baseline
      const zeroLine = chart.addSeries(LineSeries, { color: 'rgba(148, 163, 184, 0.3)', lineWidth: 1, lineStyle: 2 })
